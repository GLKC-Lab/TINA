...
function runSignalRefinementWorkflow() {
    ...
    while (keepTesting == 1) {
        ...
        continueMessage = continueMessage + "\nWould you like to evaluate another background correction setting?";
...
function processOneSignalRefinementImage(path, optDir, settingsLabel, previousSettingsLabel, rbr, lightBackground, slidingParaboloid, disableSmoothing, inputPolarity, previewLUT, applyMedianRefinement, syncComparisonWindows) {
    ...
    if (previousSettingsLabel == "") {
        ...
        inspectMessage = inspectMessage + "SYNC TIP: Click Synchronize All for linked cursor comparison. ";
        inspectMessage = inspectMessage + "Click OK to assess the suitability of this background correction setting.";
        waitForUser("Inspect preview images", inspectMessage);
    } else {
        inspectMessage = "Review previous and current preview images. ";
        inspectMessage = inspectMessage + "SYNC TIP: Click Synchronize All for linked cursor comparison. ";
        inspectMessage = inspectMessage + "Click OK to assess the suitability of this background correction setting.";
        waitForUser("Inspect current and previous results", inspectMessage);
    }
